...
edge_type = {
	...
	edge2str('batadv_priv_mcast', 'querier_ipv4', 'batadv_mcast_querier\l_state'):     EDGE_NOPOINTER,
	edge2str('batadv_priv_mcast', 'querier_ipv6', 'batadv_mcast_querier\l_state'):     EDGE_NOPOINTER,
...
def read_graph(inpath):
	...
	r_edge = re.compile('^\s*(?P<to>[A-Za-z][A-Za-z0-9]*) -> (?P<from>[A-Za-z][A-Za-z0-9]*) \[.*,label="(?P<label>[^"]+)"')
	r_node = re.compile('^\s*(?P<node>[A-Za-z][A-Za-z0-9]*) \[label="(?P<label>[^"]+)"')
	...
	for l in lines:
		...
		if m_edge:
			...
			labels = label.split('\\n')
			for l in labels:
				e = {'from': m_edge.group('from'), 'to': m_edge.group('to'), 'label': l}
				graph_raw.append(e)

	# translate graph
	for r in graph_raw:
		e = {'from': node2name[r['from']], 'to': node2name[r['to']], 'label': r['label']}
		g.append(e)
...
def write_graph(outpath, g):
	...
	for e in g:
		lines.append('"%s" -> "%s" [label=" %s", %s]' % (e['from'], e['to'], e['label'], get_edge_format(e)))
...
def remove_non_ref_edges(g):
	return filter(lambda e: get_edgetype(e) in [EDGE_KREF, EDGE_DEFAULT, EDGE_NOPOINTER], g)
...
def remove_non_cycle_nodes(g):
	...
	while True:
		...
		g = filter(lambda e: e['from'] not in removable_nodes and e['to'] not in removable_nodes, g)

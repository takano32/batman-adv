 ...
 */
static void batadv_meshif_vlan_release(struct kref *ref)
{
	struct batadv_meshif_vlan *vlan;
 ...
 */
void batadv_meshif_vlan_put(struct batadv_meshif_vlan *vlan)
{
	if (!vlan)
		return;
...
/**
 * batadv_meshif_create() - Create and register mesh interface
 * @net: the applicable net namespace
 * @name: name of the new mesh interface
 *
 * Return: newly allocated mesh_interface, NULL on errors
 */
struct net_device *batadv_meshif_create(struct net *net, const char *name)
{
	struct net_device *mesh_iface;
	int ret;

	mesh_iface = alloc_netdev(sizeof(struct batadv_priv), name,
				  NET_NAME_UNKNOWN, batadv_meshif_init_early);
	if (!mesh_iface)
		return NULL;

	dev_net_set(mesh_iface, net);

	mesh_iface->rtnl_link_ops = &batadv_link_ops;

	ret = register_netdevice(mesh_iface);
	if (ret < 0) {
		pr_err("Unable to register the batman interface '%s': %i\n",
		       name, ret);
		free_netdev(mesh_iface);
		return NULL;
	}
